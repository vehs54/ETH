...
VOLUME_CONFIRMATION_MULTIPLIER = 1.2   # current volume must exceed avg * this to confirm
...
def apply_structure_filter(signal, structure):
    """
    Blocks a confirmed BUY/SELL if any of the following clearly disagree:
      - 1H structure (e.g. don't BUY into a confirmed BEARISH 1H structure)
      - 6H structure (higher-timeframe — same logic, coarser trend)
      - Volume (require above-average volume to confirm the move isn't just noise)

    RANGING/UNKNOWN structure and unknown volume (not enough history yet)
    don't block anything — there's no clear evidence either way, so we let
    the underlying MA/RSI signal stand. They still affect confidence scoring.
    """
    trend = structure.get("trend")
    htf_trend = structure.get("htf_trend")
    volume = structure.get("volume")
